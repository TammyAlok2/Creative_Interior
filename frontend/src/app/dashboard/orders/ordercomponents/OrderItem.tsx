      ...
      <div className="w-[40rem] max-[763px]:w-full max-2xl:w-[26rem] min-2xl:w-[29rem] max-[543px]:w-full flex-shrink-0 flex gap-[1rem]">
        <div className="w-[8rem] h-[6rem]">
          <Image
            ...
            alt={item.name}
            className="object-cover rounded-lg h-full w-full scale-[0.8]"
          />
        </div>
        <div className="lg:flex-col w-full max-2xl:flex-col max-2xl:gap-[0rem] flex justify-around">
          <div className="w-full max-[763px]:flex max-[763px]:items-center max-[763px]:justify-between">
            <div>
              <h1 className="font-semibold text-lg mb-1">
                ...
                <div className="max-[764px]:block min-[763px]:hidden">
                  <div className="flex gap-1 items-center">
                    <TbPointFilled
                      className={`text-[1.2rem] relative top-[0.1rem] text-${item.statusColor}`}
                    />
                    <StatusBadge status={item.orderStatus} />
                  </div>
                  <p className="max-[560px]:hidden">
                    {item?.productId?.description?.slice(0, 30) + "..."}
                  </p>
                  <p className="max-[560px]:block max-[763px]:hidden">
                    {item?.productId?.description?.slice(0, 16) + "..."}
                  </p>
                  <div className="flex gap-[.5rem] items-center">
                    <p className="mt-1 font-semibold text-[1rem] text-gray-medium">
                      {item.date}
                    </p>
                  </div>
                </div>
              ...
              <p className="text-gray-500 text-sm max-[763px]:hidden">
                Quantity: {item.quantity}
              </p>
            </div>
            <div className="min-[763px]:hidden text-[2rem]">
              <FiChevronRight />
            </div>
          </div>
          <p className="text-gray-800 font-semibold mt-1 max-[763px]:hidden">
